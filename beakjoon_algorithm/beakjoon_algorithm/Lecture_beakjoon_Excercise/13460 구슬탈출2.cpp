//
//  13460 구슬탈출2.cpp
//  beakjoon_algorithm
//
//  Created by kgh on 16/08/2019.
//  Copyright © 2019 kgh. All rights reserved.
//
#include <iostream>
#include <vector>
#include <string>
using namespace std;
int dx[] = {0,0,1,-1};
int dy[] = {1,-1,0,0};
const int LIMIT = 10;
vector<int> gen(int k) {
    vector<int> a(LIMIT);
    // 4진법으로 변환
    for (int i=0; i<LIMIT; i++) {
        a[i] = (k&3);
        k >>= 2;
    }
    return a;
}
// 시뮬레이트 함수
pair<bool,bool> simulate(vector<string> &a, int k, int &x, int &y) {
    
    // '.'일 경우 false , false 반환
    if (a[x][y] == '.'){
        return make_pair(false, false);
    }
    // n,m 행렬 반환
    int n = a.size();
    int m = a[0].size();
    bool moved = false;
    
    // 방향을 돌아가면서 값을 바꿔서 진행한다.
    while (true) {
        int nx = x+dx[k];
        int ny = y+dy[k];
        //값에 만족하면 Moved이동한다.
        if (nx < 0 || nx >= n || ny < 0 || ny >= m) {
            return make_pair(moved, false);
        }
        // #일 경우 벽일경우
        if (a[nx][ny] == '#') {
            return make_pair(moved, false);
            // RED, BLUE Ball 일 경우
        } else if (a[nx][ny] == 'R' || a[nx][ny] == 'B') {
            return make_pair(moved, false);
        } // '.'일 경우 갈 수있는 경우니까 값을 바꿔서 넣어준다.
        else if (a[nx][ny] == '.') {
            swap(a[nx][ny], a[x][y]);
            x = nx;
            y = ny;
            moved = true;
            // 구멍에 성공적으로 넣어줘서 반환 시킴.
        } else if (a[nx][ny] == 'O') {
            a[x][y] = '.';
            moved = true;
            return make_pair(moved, true);
        }
    }
    // 둘다 아니라고 make_pair(false, false) 값 반환
    return make_pair(false, false);
}
int check(vector<string> a, vector<int> &dir) {
    
    int n = a.size();       // 행 사이즈
    int m = a[0].size();    // 열 사이즈
    int hx,hy,rx,ry,bx,by;
    // 행,렬값 체크 한다.
    for (int i=0; i<n; i++) {
        for (int j=0; j<m; j++) {
            if (a[i][j] == 'O') {
                hx = i; hy = j;
            } else if (a[i][j] == 'R') {
                rx = i; ry = j;
            } else if (a[i][j] == 'B') {
                bx = i; by = j;
            }
        }
    }
    int cnt = 0;
    // 생성된 값을 모두돌면서 순회
    for (int k : dir) {
        cnt += 1;
        bool hole1=false, hole2=false;
        while (true) {
            auto p1 = simulate(a, k, rx, ry);
            auto p2 = simulate(a, k, bx, by);
            if (p1.first == false && p2.first == false) {
                break;
            }
            if (p1.second) hole1 = true;
            if (p2.second) hole2 = true;
        }
        if (hole2) return -1;
        if (hole1) return cnt;
    }
    return -1;
}
bool valid(vector<int> &dir) {
    int l = dir.size();
    
    for (int i=0; i+1<l; i++) {
        if (dir[i] == 0 && dir[i+1] == 1){
            return false;
        }
        if (dir[i] == 1 && dir[i+1] == 0){
            return false;
        }
        if (dir[i] == 2 && dir[i+1] == 3){
            return false;
        }
        if (dir[i] == 3 && dir[i+1] == 2){
            return false;
        }
        if (dir[i] == dir[i+1]){
            return false;
        }
    }
    return true;
}
int main() {
    int n, m;
    cin >> n >> m;
    vector<string> a(n);
    for (int i=0; i<n; i++) {
        cin >> a[i];
    }
    int ans = -1;
    for (int k=0; k<(1<<(LIMIT*2)); k++) {
        
        vector<int> dir = gen(k); // 정수 -> 4진법으로 변환
        if (!valid(dir)){
          continue;  // 타당성 확인
        }
        int cur = check(a, dir);    // 값을 체크하고, ans값으로 받음.
        if (cur == -1){
          continue;
        }
        if (ans == -1 || ans > cur){
          ans = cur;
        }
    }
    cout << ans << '\n';
    return 0;
}
